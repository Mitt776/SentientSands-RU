...
def parse_event_line(line: str, current_day: Optional[int]) -> Tuple[Optional[Event], Optional[int]]:
    ...
    # Dayless lines are malformed partial writes from the live log path.
    # Keeping them here causes them to inherit a stale day and leak into
    # synthesized rumors as noisy unknown incidents.
    if not m:
        return None, current_day
    day = int(m.group("day"))
            ...
            for victim, count in sorted(unknown_inflicted_by_day_loc.get(day, {}).get(loc, {}).items()):
                ...
                if opponents:
                    violence_lines.append(
                        (count, f"{victim} suffered {count} {setback_word(count)} amid fighting with {join_with_and(sorted(opponents))}")
                    )
                else:
                    violence_lines.append((count, f"{victim} suffered {count} unattributed {setback_word(count)}"))
