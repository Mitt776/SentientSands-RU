...
_DEFAULT_SAPIENT_RACES = [
    "Greenlander", "Scorchlander", "Shek", "Skeleton", "Hiver", "Hive",
    "Hive Prince", "Soldier Drone", "Worker Drone", "Southern Hive",
    "Western Hive", "Northern Hive", "Midland Hive"
]

_DEFAULT_FERAL_RACE_KEYWORDS = [
    "deadhive", "fogman", "cannibal", "gurgl"
]

_DEFAULT_FERAL_FACTION_KEYWORDS = [
    "fogmen", "cannibal", "screamer", "gurgler"
]
...
def _resolve_persona_races_path():
    if os.path.exists(_PERSONA_RACES_PRIMARY_PATH):
        return _PERSONA_RACES_PRIMARY_PATH
    if os.path.exists(_PERSONA_RACES_LEGACY_PATH):
        return _PERSONA_RACES_LEGACY_PATH
    return _PERSONA_RACES_PRIMARY_PATH


def _load_persona_race_registry(path=None):
    defaults = {
        "animal_races": list(_DEFAULT_ANIMAL_RACES),
        "machine_races": list(_DEFAULT_MACHINE_RACES),
        "sapient_races": list(_DEFAULT_SAPIENT_RACES),
        "feral_race_keywords": list(_DEFAULT_FERAL_RACE_KEYWORDS),
        "feral_faction_keywords": list(_DEFAULT_FERAL_FACTION_KEYWORDS),
    }
    persona_races_path = path or _resolve_persona_races_path()
    try:
        if not os.path.exists(persona_races_path):
            return defaults
        with open(persona_races_path, "r", encoding="utf-8") as f:
            loaded = json.load(f)
        if not isinstance(loaded, dict):
            log.warning("persona_races.json must be a JSON object. Using built-in defaults.")
            return defaults
        merged = {}
        for key, fallback in defaults.items():
            merged[key] = _dedupe_preserve(loaded.get(key, fallback))
        return merged
    except Exception as exc:
        log.warning(f"Failed to load persona_races.json: {exc}. Using built-in defaults.")
        return defaults


_PERSONA_RACE_REGISTRY = {}

ANIMAL_RACES = []
MACHINE_RACES = []
SAPIENT_RACES = []
FERAL_RACE_KEYWORDS = []
FERAL_FACTION_KEYWORDS = []
KNOWN_RACE_TOKENS = []


def reload_persona_race_registry(path=None):
    global _PERSONA_RACE_REGISTRY, PERSONA_RACES_PATH
    PERSONA_RACES_PATH = path or _resolve_persona_races_path()
    registry = _load_persona_race_registry(PERSONA_RACES_PATH)
    _PERSONA_RACE_REGISTRY = dict(registry)
    ANIMAL_RACES[:] = list(registry["animal_races"])
    MACHINE_RACES[:] = list(registry["machine_races"])
    SAPIENT_RACES[:] = list(registry["sapient_races"])
    FERAL_RACE_KEYWORDS[:] = list(registry["feral_race_keywords"])
    FERAL_FACTION_KEYWORDS[:] = list(registry["feral_faction_keywords"])
    KNOWN_RACE_TOKENS[:] = _dedupe_preserve(
        ANIMAL_RACES + MACHINE_RACES + SAPIENT_RACES + FERAL_RACE_KEYWORDS
    )
    return dict(_PERSONA_RACE_REGISTRY)


reload_persona_race_registry()
...
def is_known_race(race):
    return race_matches_any(race, KNOWN_RACE_TOKENS)
